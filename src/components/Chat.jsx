...
function Chat() {
const { workerId, bookingWorkerName } = useParams();
    ...
    const fetchMessages = async () => {
        try {
            ...
            const currentUserData = JSON.parse(sessionStorage.getItem('existingUser'));
            ...
            if (storedToken) {
                ...
                const result = await getMessages(currentUserData._id, workerId, reqHeader);
                setMessages(result.data);
                
                // Fetch sender and receiver usernames
                setSenderUsername(currentUserData.name);
                setReceiverUsername(bookingWorkerName); // Assuming workerId is the username of the receiver
            }
        } catch (error) {
    ...
    const handleSendMessage = async () => {
        const senderId = JSON.parse(sessionStorage.getItem('existingUser'))._id;
        
        try {
            // Prepare request body and headers
            const reqBody = { senderId: senderId, receiverId: workerId, message: message };
            const reqHeader = {
                'Content-Type': 'application/json',
    ...
    return (
        <div>
            <h1>Chat with Worker {bookingWorkerName}</h1>
